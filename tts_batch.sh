...
# ---- 合成（辞書DLは初回のみ）＆ index.csv 作成（CSV安全・かな/g2p付き）----
python - <<'PY' "$infile"
import sys, csv, wave
import numpy as np
...
with open(infile, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        rows.append([f"{n:03d}", text, fn, oj.g2p(text, kana=True), oj.g2p(text)])
        print(f"saved: {fn}")
        n += 1

with open("index.csv","w",encoding="utf-8",newline="") as wf:
    w = csv.writer(wf); w.writerow(["id","text","wav_path","kana","g2p"]); w.writerows(rows)
PY
...
for f in utt_*.wav; do
  ...
  ffmpeg -y -hide_banner -loglevel error -i "$f" \
    -af "atempo=${rate},volume=${vol}dB" -ar 48000 -ac 1 -b:a 128k \
    "mp3/${base}.mp3"
done
